...
test("Shoud show a Videos field", async ({ page, context, baseURL }) => {
	...
	const length = await page
		.locator('div[data-type="plcdnl\\\\videos\\\\fields\\\\Video"]')
		.count()
...
test("Shoud show a Matrix Videos field", async ({ page, context, baseURL }) => {
	...
	// Add a matrix row
	await page
		.locator('#fields-videos.matrix button.add[data-type="video"]')
		.click()

	// Check that there is at least one video
	const length = await page
		.locator(
			'#fields-videos.matrix .fields div[data-type="plcdnl\\\\videos\\\\fields\\\\Video"]'
		)
		.count()
	expect(length > 0).toBeTruthy()
})
